...
func (r *FilesRepo) CreateFile(ctx context.Context, f repo.File) (repo.File, error) {
	const q = `
INSERT INTO files (id, name, size_bytes, sha256_hex, cid, local_path)
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING created_at
`
	err := r.pool.QueryRow(ctx, q, f.ID, f.Name, f.SizeBytes, f.SHA256Hex, f.CID, f.LocalPath).Scan(&f.CreatedAt)
	if err != nil {
...
func (r *FilesRepo) GetFileByID(ctx context.Context, id string) (repo.File, error) {
	const q = `
SELECT id, name, size_bytes, sha256_hex, cid, local_path, created_at
FROM files
WHERE id = $1
`
	var f repo.File
	err := r.pool.QueryRow(ctx, q, id).Scan(&f.ID, &f.Name, &f.SizeBytes, &f.SHA256Hex, &f.CID, &f.LocalPath, &f.CreatedAt)
	if err != nil {
...
`
	var f repo.File
	err := r.pool.QueryRow(ctx, q, cid).Scan(&f.ID, &f.Name, &f.SizeBytes, &f.SHA256Hex, &f.CID, &f.LocalPath, &f.CreatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return repo.File{}, repo.ErrNotFound
		}
...
func (r *FilesRepo) ListFiles(ctx context.Context, limit int) ([]repo.File, error) {
...
`
	...
	for rows.Next() {
		var f repo.File
		if err := rows.Scan(&f.ID, &f.Name, &f.SizeBytes, &f.SHA256Hex, &f.CID, &f.LocalPath, &f.CreatedAt); err != nil {
